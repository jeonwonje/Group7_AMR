...
The current mission stack (`auto_explore_v2` and `CDE2310_AMR_Trial_Run`) does
not ship algorithm-level unit tests. An earlier `amr_nav` package carried 29
pathfinding unit tests (`src/amr_nav/test/test_pathfinding.py`) covering
occupancy conversion, Dijkstra target finding, wall-penalised A*, and waypoint
clustering; that package was removed during the consolidation documented in
CHANGELOG 1.1.0, and the tests are recoverable from git history at commit
`044e346` but are no longer built or run.

Each shipped package retains the ament-generated lint/style tests documented in
§4 below. Algorithm validation is currently covered by the integration and
system tests in §5 and §6.
...
| TST-INT-02 | AprilTag TF broadcast              | RPi camera running, tag visible      | 1. Launch external `apriltag_ros` node on RPi<br>2. Hold tag in front of camera<br>3. `ros2 run tf2_ros tf2_echo camera_link tag36h11:0` | Transform printed with age < 0.5 s | FR-DET-02  |
...
|---------|------------------------------------------------|----------|------------|---------------------|---------------------------------------------------------------------------------------------|
| #5      | Camera flicker causes false tag detections     | Medium   | Mitigated  | apriltag_ros / mission_coordinator | Rapid lighting changes cause frame-level false positives. Mitigated by stale TF threshold (0.5 s). |
